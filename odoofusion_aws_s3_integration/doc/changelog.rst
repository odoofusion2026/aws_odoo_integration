...
Changes:
- Converted module architecture and manifests to Odoo 17 (version 17.0.2.0.0) with author Metamorphosis, Joyanto (odoofusion_aws_s3_integration/__manifest__.py)
- Ported models for global S3 config, ir.attachment storage engine hooks (_file_write, _file_read, _file_delete, _to_http_stream), transactional deletion queue, background migration wizard, dashboard, and S3 file explorer to Odoo 17 ORM standards (odoofusion_aws_s3_integration/models/)
- Created XML views for S3 configuration, delete queue monitor, migration wizard, attachment status banners, and navigation menus adapted for Odoo 17 (odoofusion_aws_s3_integration/views/)
...
Changes:
- Fixed Invalid view aws.s3.log.search definition ParseError: replaced legacy expand="0" and string="Group By" attributes on search view group element with name="group_by" per Odoo RNG schema specifications (views/aws_s3_log_views.xml)
